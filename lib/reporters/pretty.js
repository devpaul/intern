...
define([
	...
	function drawProgressBar(report, width) {
		...
		var numSize = String(report.total).length;
		var barSize = Math.min(Math.max(width - 4 - (numSize * 2), 1), report.total);
		...
		charm.write(pad(barSize - results.length) + '] ' + padLeft(report.finished, numSize) + '/' + report.total);
	}
	 ...
	 */
	function drawReporter(report, width) {
		var charm = pretty.charm;
		var rightOfBar = '' +
			(report.failed ? ', ' + report.failed + ' fail' : '') +
			(report.skipped ? ', ' + report.skipped + ' skip' : '');
		var title = abbreviateEnvironment(report.environment, pretty.titleWidth);
		var barWidth = width - rightOfBar.length - pretty.titleWidth;
