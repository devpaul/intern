...
define([
	...
	function drawProgressBar(report, width) {
		...
		results.forEach(function (value) {
			charm.write(CHAR[value]);
		});
		charm.write(pad(barSize - results.length) + '] ' +
	...
	function render() {
		var charm = pretty.charm;
		var logLength = pretty.dimensions.height - 2 - pretty.sessions.length - 1;
	...
	var pretty = {
		...
		_Report: Report,
		_drawProgressBar: drawProgressBar,
		_drawReporter: drawReporter,
		_drawTotalReporter: drawTotalReporter,
		_render: render,

		start: function () {
			pretty.charm = new Charm();
			pretty.charm.pipe(process.stdout);

			process.stdout.on('resize', function () {
				pretty.dimensions.width = process.stdout.columns;
				pretty.dimensions.height = process.stdout.rows;
			});
		...
		stop: function () {
			...
			render(); // TODO omit displaying errors

			// Display coverage information
			(new Reporter()).writeReport(pretty.total.coverage, true);

			// TODO write a full log of errors
		},
		...
		'/test/skip': function (test) {
			pretty.header = '/test/skip: ' + test.sessionId; // TODO remove
			pretty._record(test.sessionId, SKIP);
		},
		'/test/pass': function (test) {
			pretty.header = '/test/pass: ' + test.sessionId; // TODO remove
			pretty._record(test.sessionId, PASS);
		},
		'/test/fail': function (test) {
			pretty.header = '/test/fail: ' + test.sessionId; // TODO remove
			pretty._record(test.sessionId, FAIL);
		},

		'/tunnel/start': function () {
			pretty.header = 'Tunnel: Starting';
		},
		'/tunnel/download/progress': function (tunnel, progress) {
			pretty.header = 'Tunnel: Downloading ' + (progress.received / progress.total * 100).toFixed(2) + '%';
		},
		'/tunnel/status': function (tunnel, status) {
			pretty.header = 'Tunnel: ' + status;
		},

		'/error': function (error) {
			pretty.header = 'error: ' + error.message;
